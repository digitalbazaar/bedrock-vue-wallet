...
// because every render method is awaited together
...
export async function getRenderedDisplays({credential} = {}) {
  ...
  // unwrapped. A bare object silently yielded no artwork and a bare string
  // threw `.map is not a function` out of an unawaited call.
  const renderMethods = _toArray(credential?.renderMethod);
