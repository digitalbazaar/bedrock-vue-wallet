...
describe('getRenderedDisplays', () => {
  ...
  it('reads a media type declared on the render method itself', async () => {
    // earlier render-method contexts put `mediaType` on the render method;
    // later ones moved it inside `outputPreference`, and real credentials are
    // written against both.
    // the value has to differ from what the suite implies, or this passes on
    // the suite fallback whether or not the term is read at all
    const displays = await getRenderedDisplays({
      credential: credential({
        renderMethod: [svg2023({mediaType: 'image/png'})]
      })
    });
    expect(displays[0].mediaType).toBe('image/png');
  });

  it('lets outputPreference win over a media type on the render method',
    async () => {
      // the newer location is the more specific statement
      const displays = await getRenderedDisplays({
        credential: credential({
          renderMethod: [html({
            mediaType: 'text/plain',
            outputPreference: {mediaType: 'text/html'}
          })]
        })
      });
  ...
  it('reads the access mode under either spelling', async () => {
    // the render-method context names the term `mode`; the spec prose calls it
    // `accessMode`, and a credential may carry either
    const [byContext, byProse] = await getRenderedDisplays({
      credential: credential({
        renderMethod: [
          html({outputPreference: {mode: ['auditory']}}),
          html({outputPreference: {accessMode: ['tactile']}})
        ]
      })
    });
    expect(byContext.accessMode).toEqual(['auditory']);
    expect(byProse.accessMode).toEqual(['tactile']);
  });

  it('accepts a single access mode that is not in an array', async () => {
    // the term is `@container: @set`, so one value may appear unwrapped
    const displays = await getRenderedDisplays({
      credential: credential({
        renderMethod: [html({outputPreference: {mode: 'auditory'}})]
      })
    });
